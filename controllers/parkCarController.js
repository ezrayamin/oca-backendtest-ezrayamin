    ...
    resgisterCar: async (req, res) => {
        ...
        try {
            if (!plat_nomor || !warna || !tipe) {
                ...
                let tempCars = []
                dbCars.map(car => {
                    if (car.plat_nomor === upperCasePlate) {
                        tempCars.push(car)
                    }
                })

                if (tempCars.length >= 1) {
                    res.status(403).send({ peringatan: 'plat nomor ini sudah teregistrasi di parkiran' })
                }
                ...
                res.status(200).send({ plat_nomor: upperCasePlate, parking_lot, tanggal_masuk: registeredData.tanggal_masuk })
            }
        }
        catch (err) {
    ...
    checkoutCar: async (req, res) => {
        ...
        try {
            ...
            if (!plat_nomor) {
                ...
                res.status(200).send({ plat_nomor: upperCasePlate, tanggal_masuk: timeRegistered, tanggal_keluar: timeCheckout, jumlah_bayar: totalPrice.toString() })
            }
        }
        catch (err) {
            res.status(400).send(err)
